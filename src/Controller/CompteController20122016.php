...
class CompteController extends AbstractController {
     ...
     * 
     */
    public function ajoutCompteAction(): Response($locale, $idabonne = 0) {
...
//                                exit ;
     ...
     * 
     */
    public function listeCompteAction(): Response($locale) {
        $em = $this->entityManager;
        $authManager = $this->Auth.Manager; //on recupere le service qui gère l'authentification = $this->Auth.Manager;//on recupere le service qui gère l'authentification
        $this->requestStack->getCurrentRequest()->setLocale($locale);
     ...
     * 
     */
    public function faireOppositionAction(): Response($locale,$numeroCompte) {
     ...
     * @return 
     */
    public function detailCompteAction(): Response($page, $imprimer, $idCompte, $idAbonne,$post, $locale,$deb,$fin,$mttde,$mtta,$sens) {
        $em = $this->entityManager;
        $authManager = $this->Auth.Manager; //on recupere le service qui gère l'authentification = $this->Auth.Manager;//on recupere le service qui gère l'authentification
        $this->requestStack->getCurrentRequest()->setLocale($locale);
     ...
     */
    public function detailCompteAbonneAction(): Response($page, $imprimer, $idCompte,$post, $idAbonne, $locale,$deb,$fin,$mttde,$mtta,$sens) {
        $em = $this->entityManager;
        $authManager = $this->Auth.Manager; //on recupere le service qui gère l'authentification = $this->Auth.Manager;//on recupere le service qui gère l'authentification
        $this->requestStack->getCurrentRequest()->setLocale($locale);
...
//        var_dump($listeOperation);      exit;  
    ...
    public function exporterOperationCompteAction(): Response($locale, $imprimer, $cpte, $deb, $fin, $mttde, $mtta, $sens,$type) {
        ...
        $dateDeb = new \Datetime(); $dateDebut = new \Datetime(); 
        $interval30jr = new \DateInterval('P1M'); $intervalajust = new \DateInterval('P1D');        
        $dateDeb->sub($interval30jr); 
       ...
       //var_dump($soldeDeb);
        ...
        elseif ($typefichier == 3) {
            ...
            $header = array();
            $donneperation = null;
            $donneperation = array();$donneperation[0]="";
            ...
            $separateur = chr(9);
             ...
             if ( $lecpte != null && $lecpte->getTypeCompte()->getId() == 3 ){
                //$donneperation[$aux]="DATE"."" .$separateur . "" ."SIGNE"." "."MONTANT". "" .$separateur . "" . "DATE VALEUR". "" . $separateur ."          ".$separateur.""."LIBELLE "."" .$separateur . "NUMERO MVT" ."\r\n"; ; 
                $donneperation[0] .= preg_replace("#([[:digit:]]{2})([[:digit:]]{2})-([[:digit:]]{2})-([[:digit:]]{2})#","$4$3$2","DATE")."\t"."SIGNE "."MONTANT"."\t\t"."DATE VALEUR"."\t"."LIBELLE"."\t\t"."NUMERO MVT"."\r\n";
				 //"DATE"."" .$separateur . "" ."SIGNE"." "."MONTANT". "" .$separateur . "" . "DATE VALEUR". "" . $separateur ."          ".$separateur.""."LIBELLE "."" .$separateur . "NUMERO MVT" ."\r\n"; ; 
             }
    ...
    function SupprimerCompteAction($locale, $idCompte, $idAbonne) {
    ...
    public function releveAbonneAction(): Response($locale) {
        $em = $this->entityManager;
        $authManager = $this->Auth.Manager; //on recupere le service qui gère l'authentification = $this->Auth.Manager;//on recupere le service qui gère l'authentification
        $this->requestStack->getCurrentRequest()->setLocale($locale);
...
//                        ->findAllCompteAbonne($unAbonne->getId());
    ...
    public function imprimeReleveAbonneAction(): Response($locale) {

        // initilaisation
        $unAbonne = null; $listeOperation = null;
        ...
        $cpte = $request->request->get('numCompte');  $mois = $request->request->get('mois'); $an = $request->request->get('annee');
        ...
        $per = $mois . ' ' . $an;   $laperiode = $tabMois[$locale][$mois] . ' ' . $an;
        $uncompte = $em->getRepository("utbClientBundle:Compte")->find($cpte);
        $gestTel = '';$agence = null;$numrib ='';
        ...
        $lien = ''; $path = $this->entityManager->getRepository("utbClientBundle:ParamSysteme")->find(4);
        
        if ($path != null){ $lien = $path->getValeur(); } else { $path=''; }
        ...
        $page = 1;  $total = count($listeOperation); $nb_page = ceil(($total - $nbre_operation_first_page) / $operations_per_page) + 1;

        ob_start(); $donneperation = ""; 
        ...
        if($type==1){
          ...
          $t_array =  array(0=>array(
           ...
           return new Response(
            ...
            $aux = 0;
            $handle = fopen('php://memory', 'r+');
            $header = array(); $donneperation = array();
            $separateur = chr(9);$i = 1;
            $donneperation = null;
    ...
    public function infoUtilisateur(): Response($em, $authManager, $currentConnete, $user, $locale) {
        //$currentConnete = $authManager->getFlash("utb_client_data");
...
//                $listeannee = $em->getRepository('utbClientBundle:Compte')->getAnneePresentAbonne($currentUtilID,$compte);                
    ...
    public function enteteAction(): Response($locale) {
